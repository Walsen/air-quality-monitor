...
This exists because that drift actually happened. `design.md` specified `utterance: str =
Field(min_length=1, max_length=4000)` while the implementation deliberately carries none — Req
1.5 makes the maximum configured, so a pinned constraint would be a second authority. The
document and the code disagreed, and nothing failed.
...
design decisions where the document is allowed to be looser than the code (`def confirm(self) ->
Self: ...`), and asserting over them would make the guard fire on every legitimate elaboration.
"""
...
import aqm_advisor.domain.redflag as redflag_module

_SCANNED_MODULES = (models_module, records_module, redflag_module)
"""Every module whose classes the guard compares.

Add a module here as its classes are built. A module absent from this tuple is not compared at
all, so the guard's coverage would otherwise lag the code silently — which is the same failure
it exists to catch.
"""
...
def test_the_guard_compares_at_least_the_models_already_built() -> None:
    ...
    # Raised as modules are added to _SCANNED_MODULES, so coverage cannot silently shrink.
    assert len(overlap) >= 13, f"only comparing {sorted(overlap)}"
