...
to
remember with a shape that cannot hold the mistake.

**The credential and the user's prose are `SecretStr`.** Req 5.2 forbids the credential reaching
any log
entry, record, response or error message, and Req 19.2 forbids an utterance substring reaching a
log. A
convention that every call site must remember is a convention that one call site will forget, so
the values
refuse to render instead. `credential` is additionally excluded from serialisation — not masked,
absent,
because a masked placeholder still advertises that a credential was there.

**There is no `max_length` on the utterance.** Req 1.5 makes the maximum CONFIGURED, so pinning
one on the
field would create a second authority that a configured value could disagree with. That is the
same fault
Req 15.6 forbids for measurement weakness, and the same one that made Service 2's profile limits
"configured" in name only. Length is checked once, by `validate_utterance_length`, against the
value
configuration supplies.

**Every basis field is copied, never computed.** Req 9.4 says read the basis and do not
re-derive any part
of it, so these are plain carriers: no defaults that could invent a value, no validators that
could
normalise one, and no method that derives a reading from another field.
"""
...
class TurnContractError(ValueError):
    ...
    alongside
    Pydantic's own failures, rather than needing a second handler for the same class of fault.
    """
...
class UtteranceTooLongError(TurnContractError):
    ...
    text:
    rejection messages are logged, and an echoed utterance would carry the user's own words into
    a log
    that Req 19.2 forbids them from reaching. Service 2 shipped exactly this defect — a message
    that
    named the remaining hours disclosed the start time it was meant to withhold.
    """
...
def validate_utterance_length(utterance: str, *, max_length: int) -> None:
    ...
    before
    the Model_Port is invoked, which is what Req 1.5 requires — a check that ran after the model
    call
    would have already paid for the thing it was meant to prevent.
...
def _require_aware(instant: dt.datetime) -> dt.datetime:
    ...
    value
    means something read a wall clock instead, so this refuses rather than assuming a zone.
    """
...
class PriorTurn(_StrictModel):
    ...
    the guidance
    given back — the exact material Req 19.2 keeps out of logs. Making them refuse to render
    means a
    debug log of the request cannot leak the conversation, whatever the call site does.
    """
...
class AdvisoryRequest(_StrictModel):
    ...
    def _reject_blank(cls, value: str) -> str:
        ...
        explicitly.
        Trimming here rather than at a call site means the stored value and the value the
        configured
        length check sees are the same one — two differing lengths for the same request would
        make the
        limit ambiguous at the boundary.
        """
        trimmed = value.strip()
        if not trimmed:
...
class RecordReference(_StrictModel):
    ...
    def identifier(self) -> str:
        ...
        distinct —
        one sensor reports several species, and one sensor and species report at successive
        instants — so
        an audit trail keyed on it would under-report provenance while looking complete.

        None of the parts is health-adjacent: a site code, a pollutant name, an instant and a
        duration are
        public sensor facts, so storing the composite does not breach Req 20.3.
        """
...
class NowcastBasis(_StrictModel):
    ...
    different
    count of hours available, or a different weight factor give a different sub-index. Req 9.3
    requires
    that derivation to be traceable, so a basis naming only the breakpoint table left a step
    unaccounted
    for.

    This is provenance for review, NOT a second trigger for a disclosure. Req 15.6 makes the
    confidence
    Service 2 returned the single authority on how weak a measurement is, and Service 2 has
    already capped
    it for an incomplete window.
    """
...
class BasisSummary(_StrictModel):
    ...
    def record_identifiers(self) -> tuple[str, ...]:
        ...
        deriving them
        anywhere else would let the audit trail claim provenance the response never cited.
        """
    ...
    def nowcast_window_is_complete(self) -> bool | None:
        ...
        ordinary
        answer — and NOT that the window is unknown. Returning `True` for an absent nowcast
        would claim a
        completeness that was never measured; returning `False` would invent a weakness that
        does not
        exist. This is the same distinction as Service 2's correlation returning `None` on zero
        variance,
        where "nothing to compare" is not "no relationship".
...
class GuardrailEnvelope(_StrictModel):
    ...
    one, and a
    locally-composed fallback would be this service inventing a disclaimer Service 2 is
    responsible for.
    """
...
class Escalation(_StrictModel):
    ...
    def _require_a_marker(self) -> Self:
        ...
        most of
        all: an escalation with no marker could not be audited or explained to a clinician, and
        would be
...
class AdvisoryResponse(_StrictModel):
    ...
    structural rather
    than a rule a future field could break.
    """
