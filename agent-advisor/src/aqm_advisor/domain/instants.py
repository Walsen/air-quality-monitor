...
import: the engineering practices forbid importing across service directories until a shared
contract package is specced, and each service keeps its own copy covered by its own round-trip
test. The NAME is identical because the form it produces must be — a whole-second UTC instant
with a `Z` suffix.

Both functions here are pure functions of their argument. Nothing in this module can reach for
the current time, which the practices' determinism rule requires of domain code and a test
enforces: a record identifier that depended on when it was rendered would not be an identifier.
"""
...
def iso_z(instant: dt.datetime) -> str:
    ...
    a reading's identity: rounding could carry an instant into the next second, and a reading's
    identifier would then disagree with the instant Service 2 recorded.
...
def parse_iso_z(text: str) -> dt.datetime:
    ...
    allows. A lenient parser here would let a neighbouring service's differently-shaped
    timestamp through and move the disagreement downstream, where it is harder to attribute.
