...
| DD13 | **First-run onboarding is prompt behaviour over existing tools, not new state** | Req 37: the `usedDefaultProfile` signal is already surfaced into the turn, and profile_put / dated symptom_entry_put already exist, so onboarding needs no new tool, endpoint, or cross-turn session state — an incomplete profile is simply visible again next turn, which is how the multi-step offer resumes on a stateless-per-turn agent (DD11). It reuses the Req 27.2 confirm-before-write rule rather than inventing a second write path |
