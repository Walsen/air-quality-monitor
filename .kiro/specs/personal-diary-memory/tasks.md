...
- [ ] 1. Cognito stack with a template assertion
...
- [ ] 2. DynamoDB tables + serving stack configuration, with assertions
...
- [ ] 3. Association Lambda + schedule stack, with assertions
...
- [ ] 4. Register the new stacks in `infra/app.py` behind context flags
...
- [ ] 5. Chatbot `POST /login` against a fake Cognito client (TDD)
...
- [ ] 6. Chatbot `/chat` forwards the JWT to the advisor (TDD)
...
- [ ] 8. Association Lambda handler against fakes (TDD)
...
- [ ] 9. Profile + diary round-trip through the DynamoDB adapters (LocalStack)
...
- [ ] 10. Association end-to-end over LocalStack
...
- [ ] 12. Wire the new/changed checks into the Justfile and confirm offline
...
- [ ] 13. Deploy Cognito + create demo users
...
- [ ] 14. Deploy the ingestion serving stack with tables + dynamodb + cognito
...
- [ ] 15. Deploy the association Lambda + schedule; on-demand invoke
...
- [ ] 16. Redeploy the advisor at real serving
...
- [ ] 17. Redeploy the chatbot with Cognito ids
...
- [ ] 18. Demonstrate persistence, isolation, and influence
...
{
  "waves": [
    { "wave": 1, "tasks": ["1", "3", "5", "8"], "note": "repo-only, no interdependencies" },
    { "wave": 2, "tasks": ["2", "6"], "note": "2 needs 1; 6 needs 5" },
    { "wave": 3, "tasks": ["4", "7"], "note": "4 needs 1,2,3; 7 needs 6" },
    { "wave": 4, "tasks": ["9"], "note": "needs 2 and 8 (LocalStack)" },
    { "wave": 5, "tasks": ["10", "11"], "note": "need 9" },
    { "wave": 6, "tasks": ["12"], "note": "offline gate; needs 1-11" },
    { "wave": 7, "tasks": ["13"], "note": "real AWS begins; needs 12 green" },
    { "wave": 8, "tasks": ["14"], "note": "needs 13" },
    { "wave": 9, "tasks": ["15", "16"], "note": "need 14" },
    { "wave": 10, "tasks": ["17"], "note": "needs 13,16" },
    { "wave": 11, "tasks": ["18"], "note": "needs 13-17" },
    { "wave": 12, "tasks": ["19"], "note": "needs 18" }
  ]
}
```
...
```
Phase A (CDK stacks, offline)      1 ──▶ 2 ──▶ 4
                                   3 ──────────┘
Phase B (chatbot, offline)         5 ──▶ 6 ──▶ 7
Phase C (association, offline)     8
Phase D (LocalStack, fenced)       (2,8) ──▶ 9 ──▶ 10 ──▶ 11
Phase E (command surface)          (1..11) ──▶ 12
Phase F (real AWS, needs account)  13 ──▶ 14 ──▶ 15 ──▶ 16 ──▶ 17
                                   (12 gates F: offline green first)
Phase G (demo)                     (13..17) ──▶ 18 ──▶ 19
```

- Phases A, B, C are independent of each other and need only the repository.
- Phase D depends on the DynamoDB stack config (2) and the association handler (8).
...
- **Two open decisions to pin during execution** (from the design's Open
  Questions): which token the ingestion `CognitoAuthenticator` validates (id vs
  access) and the exact subject claim used as `user_id`; and whether the
  air-quality readings stay seeded/in-memory for the POC (recommended) or move to
  real DynamoDB readings.
