...
* **The job touches three stores, so three tables are wired.** ``AssociationJob``
  reads the ``profiles`` store (``get`` — to find a user's sites), the
  ``symptom-log`` store (``query_window`` for the diary AND
  ``put_learned_thresholds`` to write the result — so read AND write), and the
  ``readings`` store (``query_window`` for the exposure history — read only). The
  environment therefore selects the ``dynamodb`` adapter for all three ports and
  carries all three table names. Wiring fewer would leave the job unable to
  resolve a user's exposure and it would derive nothing.
* **IAM is scoped to exactly those three tables.** The tables are OWNED by the
  ...
  on the profiles and readings ARNs scope the DynamoDB actions to those ARNs (and
  their indexes), never ``Resource: "*"``. Importing by name keeps synth offline:
  the ARN is built from the stack's own (explicit) account and region, resolving
  nothing from a live account.
...
class AssociationStack(Stack):
    ...
    def __init__(
        ...
        *,
        ...
        #
        # AssociationJob reads three stores, so all three are the dynamodb adapter and
        # all three table names are carried. The composition reads _table("profiles")
        # -> AQM_TABLE_PROFILES, _table("readings") -> AQM_TABLE_READINGS, and
        ...
        fn_env = {
            ...
            # reads profiles + readings and reads/writes the symptom log.
        ...
        # profiles + readings: read only (get / query_window).
