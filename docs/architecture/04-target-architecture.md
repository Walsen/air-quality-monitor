...
```mermaid
flowchart TB
    subgraph edge["Edge — sensors (real or simulated)"]
        sim["Sensor Simulator swarm<br/>(sensor-simulator/)<br/>or a live public AQ feed"]
    end

    subgraph AWS["AWS · us-east-1"]
        subgraph ingest["Ingestion (PLANNED — not in the POC)"]
            iot["AWS IoT Core<br/>managed MQTT + per-device X.509"]
            rule["IoT Rules Engine<br/>SQL routing"]
            kinesis["Kinesis Data Streams<br/>(optional, at scale)"]
            ingestFn["AWS Lambda — ingest<br/>validate → dedup → calibrate →<br/>AQI + driving pollutant"]
            poller["EventBridge Scheduler → Lambda<br/>hourly pull of a live feed"]
            rawS3[("Amazon S3<br/>raw archive (immutable)")]
        end

        subgraph stores["Stores"]
            timestream[("Amazon Timestream<br/>readings time-series<br/>(PLANNED — POC uses DynamoDB)")]
            ddb[("DynamoDB<br/>profiles · symptom-log ·<br/>readings* · sensor-registry")]
        end

        subgraph serve["Serving & identity (LIVE)"]
            serveApi["API Gateway HTTP API"]
            serveFn["AWS Lambda — serving<br/>per-user join + personalization"]
            cognito["Amazon Cognito<br/>user pool"]
        end

        subgraph enrich["Enrichment (PLANNED)"]
            forecast["Forecast / pollen APIs<br/>(external, via Lambda)"]
            secrets["Secrets Manager / SSM<br/>feed + forecast API keys"]
        end

        subgraph advise["Advisor (LIVE)"]
            waf["AWS WAF<br/>(PLANNED — throttle/protect)"]
            advisor["Bedrock AgentCore Runtime<br/>AI advisor"]
            bedrock["Amazon Bedrock<br/>Claude (inference profile)"]
        end

        subgraph ui["User-facing (LIVE)"]
            chatApi["API Gateway HTTP API"]
            chatFn["AWS Lambda — web chatbot"]
        end

        subgraph sched["Scheduled work (LIVE)"]
            eventbridge["Amazon EventBridge<br/>schedule"]
            assocFn["AWS Lambda — association job<br/>diary → learned thresholds"]
        end

        obs["CloudWatch Logs + X-Ray<br/>(+ data-quality alarms — PLANNED)"]
    end
    ...
    sim -->|"MQTT + X.509"| iot
    sim -.->|"reference REST contract"| poller
    iot --> rule
    rule -->|"raw copy"| rawS3
    rule --> kinesis
    kinesis --> ingestFn
    ...
    %% serving + advisor (live), reading planned stores
    user -->|"sign in + chat (JWT)"| chatApi --> chatFn
    chatFn -->|"InitiateAuth"| cognito
    chatFn -->|"InvokeAgentRuntime (Bearer JWT)"| advisor
    waf -.-> advisor
    advisor -->|"InvokeModel"| bedrock
    advisor -->|"retrieve (Bearer JWT)"| serveApi --> serveFn
    serveFn -.->|"verify JWT (JWKS)"| cognito
    serveFn --> ddb
    serveFn -.->|"reads readings from"| timestream
    serveFn -.-> forecast
    forecast -.-> secrets

    %% scheduled association (live)
    eventbridge --> assocFn --> ddb
    ...
    classDef planned stroke-dasharray:5 5,stroke:#b58900,color:#555;
    class ingest,iot,rule,kinesis,ingestFn,poller,timestream,forecast,secrets,waf planned;
```
